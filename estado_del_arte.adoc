...
Ya se ha definido en este documento EDM como la aplicación de las técnicas tradicionales de Datamining al mundo de la educación.
La International Educational Data Mining Society, que nace en 2011 con el objetivo de fomentar la colaboración y el desarrollo científico de esta nueva disciplina, la define en su web (www.educationaldatamining.org) como "una disciplina emergente cuyo objetivo es el desarrollo de métodos para explorar los tipos de datos particulares que provienen de entornos educativos y usar esos métodos para comprender mejor a los estudiantes y los entornos en los que aprenden".
...
El proceso de Datamining es aplicable a cualquier sector o actividad que genera gran cantidad de datos.
En EDM, como ya se ha dicho, se aplicará este proceso dentro del marco educativo.

CRISP-DM (del inglés Cross Industry Standard Process for Data Mining) es un estándar abierto que define el modelo de proceso que se seguirá para proyectos de Datamining.
...
+
El proceso de Datamining empieza entendiendo el problema. Es un ejercicio conjunto que involucra a los expertos del dominio de conocimiento y de Datamining, de forma que se pueda trasladar el objetivo a la definición del problema del data mining.
+
...
. Despliegue
+
Los expertos en Datamining exportan los resultados a herramientas desde las que podrán ser explotados (como tablas de bases de datos relacionales u hojas de cálculo).
...
Esta forma está demasiado orientada al mundo web, por lo que Baker (2010) propone, no obstante, esta otra:
...
Se emplea este método cuando lo que quieres predecir es un valor numérico (por ejemplo: la nota final de 0 a 10 o el número de asignaturas que aprobará un alumno).

Para construir un modelo de regresión se parte de un conjunto de datos de entrenamiento.
Es decir, datos para los cuales conocemos el valor del atributo o variable que queremos predecir.

Junto a ese valor que queremos predecir existe otra serie de parámetros o variables que serán susceptibles de ser usados para la predicción.
