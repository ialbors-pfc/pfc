...
¿Por qué se necesita un preprocesado?
...
(Romero, Romero y Ventura, 2013)

La fase de preprocesado puede llegar a consumir hasta un 80% del tiempo dedicado a un estudio de Data Minig (Kharwar y Kapadia, 1998). Otros autores llegan a poner este valor en el 90% como ya mencionan (Gonçalves Jr, P. M., Barros, R. S., & Vieira, D. C. (2012).

Requiere de mucho tiempo pero también aporta mucho valor al estudio.
Cuanto mejores son los datos de entrada, previamente preprocesados, mayor será la calidad de los resultados y mayor probabilidad de llegar a resultados significativos y útiles.

Este preprocesado tiene más de proceso artesanal que automático.
Si bien puede haber algunas pautas generales que sirvan como guía para el mismom en general no hay un conjunto acotado y establecido de pasos a seguir.

Dependiendo del objetivo, de los datos iniciales, del algoritmo de Datamining que se quiera aplicar, de la herramienta que se usará y de otras muchas variables se aplicarán ciertas técnicas y otras.
...
** Otros descartes basado en la inspección de los datos. Por ejemplo, se ve que el país sólo es AU o está vacío. Es de suponer que no va a proporcionar mucha información.
** Los atributos de fecha. Por simplificar.
* Conversión de atributos de texto en nominales.
Por ejemplo: el atributo email no aporta en realidad mucha información (sobre todo si tenemos en cuenta que está anonimizado) pero sí podemos distinguir entre usuarios que han especificado email y usuarios que no.
