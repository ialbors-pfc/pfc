...
Este conjunto de datos se publicó con la finalidad de que fuera empleado para la investigación, está convenientemente anonimizado y además sólo contiene información de aqullos participantes que otorgaron expresamente su permiso para que se usaran con fines académicos.

Los datos se corresponden con un curso de cuatro semanas durante agosto de 2016.
En la información del conjunto de datos no se especifica la versión de Moodle que se empleó para el curso.
Si se toma en consideración las fechas de publicación de versiones y del propio curso y se hace la suposición de que el curso siempre se realiza con la última versión publicada, podríamos suponer que la versión sería 3.1.x; la primera versión 3.1.x es del 23 de mayo de 2016.

De nuevo no hay mucha diferencia con las otras versiones mencionadas en este documento y podemos suponer que no afectaría sustancialmente el uso de una u otra.

*-
El conjunto de datos consta de los siguientes ficheros
...
Correspondencia entre ficheros y las tablas de arriba


=== Análisis y extracción de datos

==== Formato arff
Weka hace uso de este formato
